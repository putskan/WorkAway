...
router.post('/setUserInfo', verifyToken, async (req, res) => {
    ...
    const emailQueryResult = await User.findOne({email: req.body.email, _id: { $ne: userID }})
    if (emailQueryResult) return res.status(400).send('Email Already Taken');
    const usernameResult = await User.findOne({username: req.body.username, _id: { $ne: userID }})
